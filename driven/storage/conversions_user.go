...
func orgAppMembershipToStorage(item model.OrgAppMembership) orgAppMembership {
	...
	return orgAppMembership{ID: id, AppOrgID: appOrgID, Permissions: permissions, Roles: roles, Groups: groups,
		Secrets: secrets, Preferences: preferences, MostRecentClientVersion: mostRecentClientVersion}
}
...
func profileFromStorage(item profile) model.Profile {
	return model.Profile{ID: item.ID, PhotoURL: item.PhotoURL, PronunciationURL: item.PronunciationURL, Pronouns: item.Pronouns,
		FirstName: item.FirstName, LastName: item.LastName, BirthYear: item.BirthYear, Address: item.Address,
		ZipCode: item.ZipCode, State: item.State, Country: item.Country, Website: item.Website, DateCreated: item.DateCreated,
		DateUpdated: item.DateUpdated, UnstructuredProperties: item.UnstructuredProperties}
}
